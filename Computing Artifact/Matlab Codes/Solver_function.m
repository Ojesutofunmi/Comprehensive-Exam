%---------------------------------------------------------------------%
%This code contains the main Solver, where the matrices and solutions are
%computed
...
%---------------------------------------------------------------------
function [q,qe,coord] = Solver_function(N,Ne,u,nu,cfl,time_final,Cf,integration_points,space_method_type,integration_type)
    ...
    Rmatrix= u*(Dmatrix - Fmatrix) + nu*Lmatrix + Cf*Mmatrix;
    ...
    time=0;
    qe = exact_solution(coord,npoin,time,u,nu,Cf);
    
    %Initialize the solution with the initial solution qe
    q=qe;
    
    %Time Integration
    [q,time] = time_integration(q,Dmatrix_hat,time,Ne,dt,coord,u,nu,Cf);
